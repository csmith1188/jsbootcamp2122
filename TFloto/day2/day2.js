...
app.get('/', (req, res) => {
  ...
  res.send(`
    ...
    <p>Attack powah: ${character.stats.attack}</p>
    <p>Defense powah: ${character.stats.defense}</p>
    <p>HP: ${character.stats.hp_current}/${character.stats.hp_max}</p></p>
    <p>inventory: ${JSON.stringify(character.inventory)}</p>
    `);
  //race and profession
  //each of the characters statements
  //one per paragraph
});
